...
// Shared by KeywordSearchService (T-2.2), SimilaritySearchService (T-2.1), and
// SearchService's Act-browse path: all three need to recover a display-ready
// section number from ActSection.SectionNumber, which ActImportService leaves
// null by design (the source data didn't reliably expose a clean per-section
// number field, so ingestion relies on OrdinalPosition for ordering instead).
// The number is usually still there, embedded as the leading token of
...
// Lives in Domain (not Infrastructure) because Application's SearchService needs it
// too, and Application can't depend on Infrastructure.
